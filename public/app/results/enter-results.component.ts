...
getDrawDetails(drawName:string)
{
    this.drawService.getDrawDetails(drawName)
        .then((drawDetails) => this.initialiseResultsArray(drawDetails))
...
saveResults(): void {
    ...
    this.drawService.saveResults(this.drawDetails.name,this.drawResults).then(()=>{this.toaster.success("Results saved");} )
                                                .catch(()=> {this.toaster.error("Failed to create results")});
    
}

handleError = (error:any):void =>
{
    this.toaster.error("An error occurred whilst fetching the draw details");
    console.log(error);
}
